...
import { initNodeClient } from "../deploy/utils/aztecUtils";
...
// Interface for Scroll API response
interface ScrollApiResponse {
    data: {
        results: Array<{ hash: string; [key: string]: any }> | null;
    };
}
...
export async function getL1ClaimDataScrollBridgeApi(l2BridgeInitiationContract: ethers.AddressLike, txHash?: ethers.BytesLike, pageSize = 10) {
    ...
    while (result === undefined) {
        ...
        const apiResJson = await apiRes.json() as ScrollApiResponse
